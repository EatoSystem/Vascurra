...
const capacityInputs=["AI + Compute","Research","Systems Development","Experts","Co-design","Open Knowledge"] as const;
export function FundHeroVisual(){return <figure className={`${styles.visual} ${styles.fundHeroVisual}`} data-artwork-key="fund-capacity-engine"><figcaption className={styles.srOnly}>Six resource streams converge into Vascurra and flow outward into learning, systems, research and products.</figcaption><Ribbon/><ul className={styles.capacityInputs}>{capacityInputs.map(item=><li key={item}>{item}</li>)}</ul><div className={styles.capacityCore}><span>Capacity engine</span><strong>Vascurra</strong></div><ul className={styles.capacityOutputs}>{["Learning","Systems","Research","Products"].map(item=><li key={item}>{item}</li>)}</ul></figure>}

export function FundAllocationVisual({items}:{items:NonNullable<StrategicChapter["items"]>}){return <figure className={`${styles.visual} ${styles.allocationVisual}`} data-artwork-key="fund-allocation"><figcaption className={styles.srOnly}>Funding capacity branches into six accountable areas of work.</figcaption><div className={styles.allocationSource}>Sustained capacity</div><ol>{items.map((item,index)=><li key={item.title}><span>{String(index+1).padStart(2,"0")}</span><div><strong>{item.title}</strong>{item.body?<small>{item.body}</small>:null}</div></li>)}</ol></figure>}

export function FundHorizons({steps}:{steps:readonly string[]}){return <figure className={`${styles.visual} ${styles.horizonVisual}`}><figcaption className={styles.srOnly}>Capability grows from core capacity through research and validated products toward global infrastructure.</figcaption><ol>{steps.map((step,index)=><li style={{"--horizon":index+1} as CSSProperties} key={step}><span>{String(index+1).padStart(2,"0")}</span><strong>{step}</strong></li>)}</ol></figure>}

export function FundFlywheel({steps}:{steps:readonly string[]}){return <figure className={`${styles.visual} ${styles.flywheelVisual}`} data-artwork-key="fund-flywheel"><figcaption className={styles.srOnly}>An open continuous loop connects support, capability, knowledge, products, value and renewed research capacity.</figcaption><Ribbon/><ol>{steps.map((step,index)=><li key={step}><span>{String(index+1).padStart(2,"0")}</span><strong>{step}</strong></li>)}</ol><div className={styles.flywheelGap}>Aspirational loop<br/><span>open to evidence</span></div></figure>}

export function FundTransparencyVisual(){const items=["Questions investigated","Evidence reviewed","Compute funded","Expert reviews","System iterations","Open outputs"] as const;return <figure className={`${styles.visual} ${styles.transparencyVisual}`}><figcaption>Future reporting categories — no current metrics</figcaption><ul>{items.map(item=><li key={item}><span aria-hidden="true"/><strong>{item}</strong><small>Capability category</small></li>)}</ul></figure>}
