...
import type { CapitalHorizon, FundFlywheelStage } from "@/content/fund";
...
export function FundAllocationVisual({items}:{items:NonNullable<StrategicChapter["items"]>}){return <figure className={`${styles.visual} ${styles.allocationVisual}`} data-artwork-key="fund-allocation"><figcaption className={styles.srOnly}>Mission capacity branches into accountable areas of work.</figcaption><div className={styles.allocationSource}>Sustained capacity</div><ol>{items.map((item,index)=><li key={item.title}><span>{String(index+1).padStart(2,"0")}</span><div><strong>{item.title}</strong>{item.body?<small>{item.body}</small>:null}</div></li>)}</ol></figure>}

export function FundHorizons({horizons}:{horizons:readonly CapitalHorizon[]}){return <figure className={`${styles.visual} ${styles.horizonVisual}`}><figcaption className={styles.srOnly}>Four planning horizons grow from a focused foundation through a research engine and international network toward permanent global capacity.</figcaption><ol>{horizons.map((horizon,index)=><li style={{"--horizon":index+1} as CSSProperties} key={horizon.id}><span>{horizon.status}</span><strong>{horizon.amount}</strong><small>{horizon.title}</small></li>)}</ol></figure>}

export function FundFlywheel({stages}:{stages:readonly FundFlywheelStage[]}){return <figure className={`${styles.visual} ${styles.flywheelVisual}`} data-artwork-key="fund-flywheel"><figcaption className={styles.srOnly}>An open continuous capital loop connects diversified resources, capability, knowledge, products, value and renewed research and development capacity.</figcaption><Ribbon/><ol>{stages.map((stage,index)=><li key={stage.label}><span>{String(index+1).padStart(2,"0")}</span><strong>{stage.label}</strong><small>{stage.items.join(" · ")}</small></li>)}</ol><div className={styles.flywheelGap}>Reinvest.<br/>Repeat.<br/><span>Long-term mission model</span></div></figure>}
