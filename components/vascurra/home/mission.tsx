  ...
  return (
    <section id="mission" aria-labelledby="mission-heading" className={`${styles.section} ${styles.missionV3}`}>
      <div className={styles.inner}>
        ...
        </div><div className={styles.missionV3Visual}><ol className={styles.missionV3Principles}>{homepageV2.mission.pillars.map(([label, body]) => <li key={label}><h3>{label}</h3><p>{body}</p></li>)}</ol><div className={styles.missionV4Artwork}><V4ArtworkSlot sizes="(max-width: 639px) 100vw, (max-width: 1199px) 92vw, 64vw" slot="mission-flow" source={v4Artwork.mission} /></div></div></div>
      </div>
    </section>
  );
}
