    ...
    <section id="support" aria-labelledby="support-heading" className={`${styles.section} ${styles.supportChapter}`}>
      <div className={styles.inner}>
        <div className={styles.supportLayout}>
          ...
          <ol className={styles.supportAreas} aria-label="What support enables">{supportAreas.map(([name, body], index) => <li key={name}><span aria-hidden="true">0{index + 1}</span><h3>{name}</h3><p>{body}</p></li>)}</ol>
        </div>
