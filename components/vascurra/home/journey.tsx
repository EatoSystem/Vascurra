...
export function Journey() {
  return <section id="journey" aria-labelledby="journey-heading" className={styles.section}><div className={`${styles.inner} ${styles.centered}`}><p className={styles.eyebrow}>Our journey</p><h2 id="journey-heading" className={styles.heading}><span>Start with one person.</span><VascurraGradientText>Learn deeply. Build for many.</VascurraGradientText></h2><p className={`${styles.lead} ${styles.centerCopy}`}>Vascurra begins with deep co-design and lived experience, with the long-term ambition of building knowledge and tools capable of helping many more people.</p><figure className={styles.journeyArtwork}><Image src="/vascurra/homepage/candidates/luminous_network_of_human_potential.webp" alt="One central person branches through an organic network toward many people and perspectives." width={1672} height={941} sizes="(min-width: 1536px) 1472px, (min-width: 640px) calc(100vw - 4rem), calc(100vw - 2.5rem)" className={styles.journeyArtworkImage} /></figure><p className={styles.progression}>{homepageV2.journey.progression.map((step, index) => <span key={step}>{index > 0 ? <b aria-hidden="true">→</b> : null}{step}</span>)}</p></div></section>;
}
