# Vascurra Repository Documentation Bundle

> Historical manifest for the initial documentation bundle. The repository now also contains the Next.js application, runtime assets, current governance documents and CI. Use Git rather than this file as the complete file inventory.

Generated files: 35

- `.cursor/rules/vascurra.mdc`
- `.github/ISSUE_TEMPLATE/feature-request.md`
- `.github/PULL_REQUEST_TEMPLATE.md`
- `.gitignore`
- `AGENTS.md`
- `CLAUDE.md`
- `README.md`
- `docs/README.md`
- `docs/brand/brand-guidelines.md`
- `docs/brand/content-voice.md`
- `docs/brand/motion-guidelines.md`
- `docs/business/business-model.md`
- `docs/business/funding-strategy.md`
- `docs/business/go-to-market.md`
- `docs/clinical/clinical-safety.md`
- `docs/clinical/consent-capacity-control.md`
- `docs/clinical/intended-purpose-and-claims.md`
- `docs/development/acceptance-criteria.md`
- `docs/development/development-workflow.md`
- `docs/development/phase-1-implementation-plan.md`
- `docs/operations/decision-log.md`
- `docs/operations/risk-register.md`
- `docs/operations/roadmap.md`
- `docs/product/phase-0-co-design.md`
- `docs/product/phase-1-website-spec.md`
- `docs/product/product-architecture.md`
- `docs/product/product-vision.md`
- `docs/product/user-journeys.md`
- `docs/regulatory/privacy-gdpr.md`
- `docs/regulatory/regulatory-strategy.md`
- `docs/research/evidence-methodology.md`
- `docs/technical/ai-provider-abstraction.md`
- `docs/technical/architecture.md`
- `docs/technical/future-data-model.md`
- `docs/technical/security.md`
